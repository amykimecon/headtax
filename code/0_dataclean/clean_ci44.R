...
parsed_clean <- parsed_wide %>%
  mutate(
    ...
    EndorsedCertNumber_Clean = as.numeric(str_replace_all(EndorsedCertNumber, "[^0-9]","")),
    EndorsedCertType_Clean = ifelse(!is.na(EndorsedCertNumber_Clean), sapply(EndorsedCertType, cert_type_extract), 0),
    # cleaning 'cert' number and type fields
    CertNumber_Clean = as.numeric(str_match(str_replace_all(CertNumber, "[^0-9]",""), "[0-9]{1,5}")),
    CertType_Clean = ifelse(!is.na(CertNumber_Clean), sapply(CertType, cert_type_extract), 0),
...
parsed_bycert <- parsed_clean %>%
  ...
  mutate(OtherCI_ci44 = str_remove(paste0(
    ifelse(CertType_Clean == 0 & !is.na(CertNumber_Clean), paste0("|", as.character(CertNumber_Clean)), ""),
    ifelse(EndorsedCertType_Clean == 0 & !is.na(EndorsedCertNumber_Clean), paste0("|", as.character(EndorsedCertNumber_Clean)), ""),
    ifelse(ArrivalCert1_Type_Clean == 0 & !is.na(ArrivalCert1_Number_Clean), paste0("|", as.character(ArrivalCert1_Number_Clean)), ""),
    ifelse(ArrivalCert2_Type_Clean == 0 & !is.na(ArrivalCert2_Number_Clean), paste0("|", as.character(ArrivalCert2_Number_Clean)), "")
  ), "^\\|")
  )%>%
  separate_wider_delim("OtherCI_ci44", "|", 
                       names = paste0("OtherCI",c(1:4),"_ci44"),
                       too_few = "align_start") %>%
  mutate(across(starts_with("OtherCI"), ~ifelse(.x=="", NA, as.numeric(.x)))) %>%
...
other_cols_ci44 = names(parsed_bycert)[which(str_detect(names(parsed_bycert), "OtherCI"))]
other_cols_reg = c("OtherCI_reg")
all_cols_ci44 = c(paste0("CI",c("4","4EXTRA","5","5EXTRA","28","28EXTRA",
                                "30","30EXTRA","36","36EXTRA"), "_ci44"), other_cols_ci44)
...
# finding unmatched
unmatched <- parsed_bycert %>% filter(!(ID_ci44 %in% merge_names_post$ID_ci44))
...
# looking at matches
View(merge_names_post %>% arrange(ID_ci44) %>% #filter(!is.na(Name_reg) & entry_match_score < 3) %>%
       select(c(ID_ci44, ID_reg, 
                total_score, nonname_score, namesim, cert_match, true_match, entry_match_score, birth_match_score, mergedon,
