...
reg_chi <- read_csv(glue("{dbox}/cleaned/chireg.csv")) %>% mutate(source = "Chinese Register", group = "Chinese Imm.")

## census data
can_all <- read_csv(glue("{dbox}/cleaned/census_all.csv"), guess_max = 5715448) %>% mutate(group = "All", source = "CA Census")
can_imm <- can_all %>% filter(IMM == 1) %>% mutate(group = "Immigrants")
can_chi <- can_all %>% filter(BORNCHI == 1) %>% mutate(group = "Chinese Imm.")

## us census data
us_all <- read_csv(glue("{dbox}/cleaned/us_clean.csv")) %>% mutate(group = "All", source = "US Census")
us_imm <- us_all %>% filter(IMM == 1) %>% mutate(group = "Immigrants")
us_chi <- us_all %>% filter(BORNCHI == 1) %>% mutate(group = "Chinese Imm.")


########################################################################
### SUMMARY STATS BY DATA SOURCE
########################################################################
us_summ <- us_all %>% mutate(EARN = NA) %>% select(c(source, group, MALE, MAR, AGE, CANREAD, EARN, LABOR, YRIMM)) %>%
  group_by(source, group) %>% summarize(across(everything(), .fns = c(~round(mean(.x, na.rm=TRUE), 2), # mean
                                                                    ~ifelse(is.na(mean(.x,na.rm=TRUE)),
                                                                            NA,
                                                                            paste0("(", round(sd(.x, na.rm=TRUE), 2), ")")))), # sd
                                        OBS = n())
can_summ <- can_all %>% select(c(source, group, MALE, MAR, AGE, CANREAD, EARN, LABOR, HOUSEOWN, RURAL, YRIMM, WEIGHT)) %>%
...
us_imm_summ <- us_imm %>% mutate(EARN = NA) %>% select(c(source, group, MALE, MAR, AGE, CANREAD, EARN, LABOR, YRIMM)) %>%
  group_by(source, group) %>% summarize(across(everything(), .fns = c(~round(mean(.x, na.rm=TRUE), 2), # mean
                                                                      ~ifelse(is.na(mean(.x,na.rm=TRUE)),
                                                                              NA,
                                                                              paste0("(", round(sd(.x, na.rm=TRUE), 2), ")")))), # sd
...
summ_stats <- bind_rows(can_imm, can_chi, us_chi %>% mutate(WEIGHT = 1), reg_chi %>% mutate(WEIGHT = 1, YRIMM = YEAR_ARRIV)) %>% 
  select(c(source, group, MALE, MAR, AGE, CANREAD, EARN, LABOR, HOUSEOWN, RURAL, YRIMM, WEIGHT)) %>%
  group_by(source, group) %>% summarize(across(-c(WEIGHT), .fns = c(~round(weighted.mean(.x, WEIGHT, na.rm=TRUE), 2), #weighted mean
                                                            ~ifelse(is.na(mean(.x,na.rm=TRUE)),
                                                                    NA,
                                                                    paste0("(", round(sqrt(wtd.var(.x, WEIGHT, na.rm=TRUE)), 2), ")")))), #weighted sd
  ...
  select(c(source, group, starts_with("MALE"), starts_with("MAR"), starts_with("AGE"), starts_with("CANREAD"), starts_with("LABOR"), starts_with("YRIMM"))) %>%
  arrange(c(source, group))
...
#                                 OBS = n())
summ_stats_out_census <- t(summ_stats)[,c(1,3,2)] %>% as.data.frame()
