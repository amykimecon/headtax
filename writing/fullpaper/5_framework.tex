...
% While this model, based on the \citet{roy1951} selection model and first used by \citet{borjas1987} to explain immigration patterns, 

% The foundational model used to explain selection into migration is the Roy-Borjas model, 

% Assuming initial negative selection into migration, the effect of the Head Tax on the skill composition of Chinese immigrants to Canada could have worked in two potential ways. First, the increased cost would have decreased the net returns to migration for everyone, such that only the most negatively selected (those with the most to gain from migration) would still choose to immigrate -- resulting in selection becoming more negative. Alternatively, if the same head tax resulted in higher costs for lower-skilled migrants (as would be predicted by liquidity constraints or skill-based exemptions from the Head Tax), an increase in the Head Tax would have disproportionately affected lower-skilled migrants, possibly resulting in selection becoming more positive.

% For prospective Chinese migrants, who were likely facing lower returns to skill in Canada relative to China, the wage gains from migrating were likely decreasing in skill. An increase in migration cost would have reduced the net gains from migration for all prospective migrants equally, so that 
\textcolor{teal}{To formalize the contrasting predictions under these two frameworks, I start with the standard Roy-Borjas model of selection into migration under which migration costs are constant, then introduce the Chiquiar-Hanson extension to consider skill-varying migration costs \citep{roy1951,borjas1987,chiquiarhanson2005}.\footnote{I follow the notation of \citet{chiquiarhanson2005} throughout.} Both models frame the decision to migrate as dependent on the wage gains to migrating net of migration cost.} In particular, a prospective Chinese migrant will choose to immigrate to Canada iff
